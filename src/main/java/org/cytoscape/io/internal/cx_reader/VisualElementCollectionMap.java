...
final public class VisualElementCollectionMap {
	...
	public final void addAnnotationsElement(final Long view,
			final CyAnnotationsElement annotation_element) {
		checkForView(view);
		String type = annotation_element.getProperties().get("type");
		if (org.cytoscape.view.presentation.annotations.ArrowAnnotation.class.toString().equals(type)) {
			view_data.get(view).getArrowAnnotationElementsList().add(annotation_element);
		} else if (org.cytoscape.view.presentation.annotations.GroupAnnotation.class.toString().equals(type)) {
			view_data.get(view).getGroupAnnotationElementsList().add(annotation_element);
		} else {
			view_data.get(view).getAnnotationElementsList().add(annotation_element);
		}
	}
	...
	public final List<CyAnnotationsElement> getAnnotationElementsList(final Long view){
		if (!view_data.containsKey(view)) {
			return null;
		}
		return view_data.get(view).getAnnotationElementsList();
	}
