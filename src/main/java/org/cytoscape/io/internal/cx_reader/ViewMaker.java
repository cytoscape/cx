...
public final class ViewMaker {
	...
	public final static void makeView(final CyNetworkView view,
		...
		addAnnotations(view, cx_view_id, collection, annotation_manager, annotation_factory_manager);
	...
	private static CyNetworkView addAnnotations(CyNetworkView network_view, 
			long cx_view_id,
			VisualElementCollectionMap collection, 
			AnnotationManager annotationManager,
			AnnotationFactoryManager annotation_factory_manager) {
		List<CyAnnotationsElement> annotationElements = collection.getAnnotationElementsList(cx_view_id);
		if (annotationElements != null) {
			for (CyAnnotationsElement annotationElement : annotationElements) {
				String type = annotationElement.getProperties().get("type");
				annotationElement.getProperties().put("view", network_view.getSUID().toString());
				annotationElement.getProperties().put("uuid", annotationElement.getUUID());
				Annotation annotation = annotation_factory_manager.createAnnotation(type, network_view, annotationElement.getProperties());
				annotationManager.addAnnotation(annotation);
				System.out.println("Created annotation in view with SUID: " + network_view.getSUID() + " new annotation count: " + annotationManager.getAnnotations(network_view).size());
			}
		}
		return network_view;
	}
